(function(){
  ...
  function renderHeader(){const ids=['election','diet','cabinet','court','local'],n=ids.filter(x=>state.cleared.includes(x)).length;$('#shardCount').textContent=n+' / 5';$('#shardMini').innerHTML=ids.map(id=>`<span class="mini-shard ${state.cleared.includes(id)?'found':''}" title="${shardNames[id]}"></span>`).join('');$('#continueBtn').disabled=!localStorage.getItem(SAVE);const sb=$('#soundBtn');if(sb){sb.textContent=soundOn?'♪':'×';sb.setAttribute('aria-label',soundOn?'音を切る':'音を出す');sb.setAttribute('aria-pressed',String(soundOn));}}
